...
def predictForm():
    ...
    telco_data_dummies = telco_data_dummies.reindex(
        columns=[
            "SeniorCitizen",
            "MonthlyCharges",
            "TotalCharges",
    ...
    telco_data_dummies = telco_data_dummies.fillna(0)
    ...
    shap_values = explainer.shap_values(telco_data_dummies)

    # Calculate the summary importance of each feature
    feature_importances = np.abs(shap_values).mean(axis=0)
    feature_names = telco_data_dummies.columns

    # Create a DataFrame to display the feature importances
    feature_importance_df = pd.DataFrame(
        {"Feature": feature_names, "Importance": feature_importances.reshape(-1)}
    )
    ...
    explanations = {
        ...
        "TotalCharges": "Less total charges.",
...
def predictUpload():
    ...
    telco_base_data = pd.read_excel(excel_file)
    ...
    # Predict the probability for all rows
    probabilities = (model.predict_proba(telco_data_dummies)[:, 1]) * 100

    telco_data_dummies["probability"] = probabilities
    telco_data_dummies["predictions"] = predictions
    telco_data_dummies["probability"]
    ...
    # Sort the filtered_df DataFrame in descending order of probability
    filtered_df = filtered_df.sort_values("probability", ascending=False)
    # Get the feature names from telco_data_dummies
    feature_names = telco_data_dummies.columns.tolist()

    # Create an explainer object using the loaded model and the telco_data_dummies
    explainer = shap.Explainer(model, telco_data_dummies)

    # Initialize an empty list to store the top features for each row
    top_features_list = []
    ...
    for index, row in filtered_df.iterrows():
        ...
        # Calculate SHAP values for the row
        ...
        for index, row in top_features.iterrows():
            ...
            importance = row["Importance"]
            percentage = importance * 100
            o3 += "{}  : {:.2f}%  ,   ".format(feature, percentage)

        # Append the feature-value pairs to the top_features_list
        top_features_list.append(o3)

    # Add the "reasons" column to the filtered_df DataFrame
    filtered_df["reasons"] = top_features_list
    ...
    columns = ["customerID", "reasons", "probability"]
